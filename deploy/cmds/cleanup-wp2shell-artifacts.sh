...
# with their revisions/postmeta/term relationships; any oembed_cache posts
# that fall within that same attack window (a pure, auto-regenerating cache,
# safe to remove wholesale on a low-traffic site rather than assessing each
# one individually); and any orphaned nav-menu-item postmeta left over from
# partial manual cleanup of injected menu items.
...
if ( $ids ) {
    ...
    // Use these posts' real timestamps (post_modified, since post_date is the
    // tool's placeholder) to establish the attack window.
    $window = $wpdb->get_row( "SELECT MIN(post_modified) AS start, MAX(post_modified) AS end FROM $wpdb->posts WHERE ID IN ($in)" );
    ...
    if ( $window && $window->start && $window->end ) {
        $oembed_ids = $wpdb->get_col( $wpdb->prepare(
            "SELECT ID FROM $wpdb->posts WHERE post_type = 'oembed_cache' AND post_date BETWEEN %s AND %s",
            $window->start,
            $window->end
        ) );
        if ( $oembed_ids ) {
            ...
            echo "Removed " . count( $oembed_ids ) . " oembed_cache posts from the attack window ($window->start to $window->end).\n";
        }
    }
}
